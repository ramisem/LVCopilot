...
import com.labvantage.opal.sql.mss.Audit;
import com.labvantage.opal.sql.mss.Certification;
import com.labvantage.opal.sql.mss.Coc;
import com.labvantage.opal.sql.mss.DataDetailView;
import com.labvantage.opal.sql.mss.Query;
import com.labvantage.opal.sql.mss.StorageUnit;
...
implements SQLGenerator {
    ...
    public MSSSqlGenerator(String connectionId, String dbms) {
        this.__ConnectionId = connectionId;
        this.__DBMS = dbms;
    }

    public MSSSqlGenerator(PageContext pageContext, String dbms) {
        this.__PageContext = pageContext;
        this.__DBMS = dbms;
    }

    public MSSSqlGenerator() {
        this.__DBMS = "MSS";
    }
    ...
    public SafeSQL getDatasetKey(SDI sdi) {
        StringBuffer sql = new StringBuffer();
        SafeSQL safeSQL = new SafeSQL();
        sql.append("SELECT PARAMLISTID + PARAMLISTVERSIONID + VARIANTID + CAST( DATASET AS VARCHAR ) DATASET");
    ...
    public SafeSQL getNotReleasedCount(SDI sdi, String dataset) {
        ...
        sql.append(" AND KEYID3 = ").append(safeSQL.addVar(sdi.getKeyid3()));
        sql.append(" AND RELEASEDFLAG <> 'Y'");
        sql.append(" AND PARAMLISTID + PARAMLISTVERSIONID + VARIANTID + CAST( DATASET AS VARCHAR ) = ").append(dataset);
        safeSQL.setPreparedSQL(sql.toString());
        return safeSQL;
    }
    ...
    public SafeSQL getSdiDatasets(SDI sdi) {
        StringBuffer sql = new StringBuffer();
        SafeSQL safeSQL = new SafeSQL();
        sql.append("SELECT PARAMLISTID, PARAMLISTVERSIONID, VARIANTID, DATASET, ISNULL( S_DATASETSTATUS, '' ) S_DATASETSTATUS");
    ...
    public SafeSQL getRemeasureInstance(SDI sdi, String paramlistid, String paramlistversionid, String variantid, String dataset) {
        StringBuffer sql = new StringBuffer();
        SafeSQL safeSQL = new SafeSQL();
        sql.append("SELECT ISNULL( T1.S_REMEASUREINSTANCE, '0' ) S_REMEASUREINSTANCE");
    ...
    public SafeSQL getSdiDataitemForTypeAndDs(String dataset, String paramType) {
        return com.labvantage.opal.sql.mss.Chart.getSdiDataitemForTypeAndDs(dataset, paramType);
    }
    ...
    public SafeSQL getSampleDetails(String sampleIdList) {
        return com.labvantage.opal.sql.common.Certification.getSampleDetails(sampleIdList);
    }

    @Override
    public SafeSQL getParamlistDetails(String paramlistIdList) {
        return com.labvantage.opal.sql.common.Certification.getParamlistDetails(paramlistIdList);
    }

    @Override
    public SafeSQL getUserCertificationDetails(String analystIdList, String paramlistIdList) {
        return Certification.getUserCertificationDetails(analystIdList, paramlistIdList);
    }

    @Override
    public SafeSQL getInstrumentDetails(String instrumentIdList) {
        return com.labvantage.opal.sql.common.Certification.getInstrumentDetails(instrumentIdList);
    }

    @Override
    public SafeSQL getInstrumentCertificationDetails(String instrumentIdList) {
        return Certification.getInstrumentCertificationDetails(instrumentIdList);
    }
    ...
    public SafeSQL getQCDataPointsForEvaluation(String batchSampleTypeId, String batchQueryCount, String paramtype, String qcbatchids, String currentBatchId, boolean chart, String sdcId, String tableId, String keyColId) {
        StringBuffer sql = new StringBuffer();
        SafeSQL safeSQL = new SafeSQL();
        sql.append(" select ds.s_qcbatchid, qcbi.s_qcbatchitemid, ds.sdcid, ds.keyid1, ds.keyid2, ds.keyid3, ds.paramlistid,");
        sql.append(" ds.paramlistversionid, ds.VARIANTID, ds.dataset, di.paramid, di.paramtype, ");
        ...
        sql.append(" from ( select distinct top ").append(batchQueryCount).append(" s." + keyColId + ", s.createdt, bi.s_qcbatchid from ");
        sql.append(tableId).append(" s, s_qcbatchitem bi, sdidata bids, s_qcbatchsampletype qcbst where s." + keyColId + " = bids.keyid1 ");
        ...
        sql.append(" order by s.createdt desc ) sss,");
        ...
        sql.append(" isnull(s_paramlisttype,'XXX') <> 'Preparation' and di.calcexcludeflag != 'Y' and ds.s_datasetstatus != 'Cancelled'");
        if (chart) {
    ...
    public String getQCDataSets(String qcBatchId, SafeSQL safeSQL) {
        ...
        sql.append(" AND LEN( T2.QCBATCHSAMPLETYPEID ) > 0");
    ...
    public SafeSQL getDataItemSpecSql(String _sdcId, String _keyid1, String _keyid2, String _keyid3, String _paramid, String _paramlistid, String _paramlistversionid, String _variantid, String _dataset, String _paramtype, String _replicateid, String _specid, String _specversionid) {
        return SdiDetailView.getDataItemSpecSql(_sdcId, _keyid1, _keyid2, _keyid3, _paramid, _paramlistid, _paramlistversionid, _variantid, _dataset, _paramtype, _replicateid, _specid, _specversionid, false);
    }
