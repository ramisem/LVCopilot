...
public class Query {
    ...
    public static SafeSQL getQueryAndArgDetails(String queryid, String basedOnSdcId) {
        ...
        sbSql.append("SELECT * FROM query ");
        sbSql.append("LEFT OUTER JOIN queryarg ");
        sbSql.append("ON query.queryid = queryarg.queryid ");
        sbSql.append("AND query.basedonid = queryarg.basedonid ");
        sbSql.append("WHERE query.queryid = " + safeSQL.addVar(queryid) + " ");
        sbSql.append("AND query.basedonid = " + safeSQL.addVar(basedOnSdcId) + " ");
        sbSql.append("ORDER BY queryarg.usersequence");
    ...
    public static SafeSQL getQueryAndArgDetails2(String queryid, String basedOnSdcId) {
        StringBuffer sbSql = new StringBuffer();
        SafeSQL safeSQL = new SafeSQL();
        sbSql.append("SELECT q.selectclause, q.fromclause, NVL (q.whereclause,'') whereclause,");
        sbSql.append("  NVL (q.orderbyclause, '') orderbyclause, qa.arginto ");
    ...
    public static SafeSQL getQueryDetails(String queryid) {
        ...
        SafeSQL safeSQL = new SafeSQL();
        sql.append("SELECT T1.SELECTCLAUSE, T1.FROMCLAUSE, NVL(T1.WHERECLAUSE, '') WHERECLAUSE,");
        sql.append(" NVL(T1.ORDERBYCLAUSE, '') ORDERBYCLAUSE");
