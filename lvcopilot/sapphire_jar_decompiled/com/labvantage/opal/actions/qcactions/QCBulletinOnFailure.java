...
public class QCBulletinOnFailure
extends QCBatchBaseAction {
    ...
    public int processAction(String actionid, String actionversionid, HashMap props) {
        if (actionid.equals("QCBulletinOnFailure")) {
            this.rc = this.doQCBulletinOnFailure(props);
        }
        return this.rc;
    }

    private int doQCBulletinOnFailure(HashMap props) {
        int rc = 1;
        ...
        try {
            ...
            String failCondition = OpalUtil.getSpecCondition(specInterpretationMap, "Fail");
            if (failCondition.length() == 0) {
                failCondition = "Fail";
            }
            ...
            actionprops.put("description", tp.translate("QCBatch Failure Bulletin"));
            StringBuffer bulletinBody = new StringBuffer();
            if (ruleEvaluationDisposition != null && ruleEvaluationDisposition.equals("Fail")) {
                bulletinBody.append(tp.translate("QC Rule Evaluation") + " ");
            }
            if (specCondition != null && specCondition.equals(failCondition)) {
