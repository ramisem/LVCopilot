...
public class SDIDocument
extends BaseSDIDetail {
    static final String LABVANTAGE_CVS_ID = "$Revision: 79326 $";

    public SDIDocument(PropertyList element, PageContext pageContext, RequestContext requestContext, String connectionid) {
        super(element, pageContext, requestContext, connectionid);
        this._TableMetaData = OpalUtil.getColumnDataTypeMap("sdidocument", this.getQueryProcessor());
        this.setPropertyHandlerClass("com.labvantage.opal.elements.sdidetailmaint.handler.SDIDocumentPropertyHandler");
    }
    ...
    public DataSet getDataset(RequestContext context) throws SapphireException {
        ...
        for (int i = 0; i < columnslist.size(); ++i) {
            ...
            if (dotindex != -1) {
                ...
                if (tableid.equalsIgnoreCase("documentid")) {
                    tableid = "DOCUMENT";
                }
        ...
        sb.append(" SDIDOCUMENT.SDCID = ").append(safeSQL.addVar(this.getSdcid()));
        sb.append(" AND SDIDOCUMENT.KEYID1 = ").append(safeSQL.addVar(this.getResolvedKeyid1()));
        sb.append(" AND SDIDOCUMENT.KEYID2 = ").append(safeSQL.addVar(this.getResolvedKeyid2()));
        sb.append(" AND SDIDOCUMENT.KEYID3 = ").append(safeSQL.addVar(this.getResolvedKeyid3()));
        sb.append(" AND DOCUMENT.DOCUMENTID = SDIDOCUMENT.DOCUMENTID");
        sb.append(" AND DOCUMENT.DOCUMENTVERSIONID = SDIDOCUMENT.DOCUMENTVERSIONID");
        String whereClause = this.element.getProperty("whereclause");
        if (whereClause != null && whereClause.trim().length() > 0) {
            sb.append(" AND ").append(whereClause);
        }
        String query = SDIDocument.getSelectSQL(columnList, "SDIDOCUMENT, DOCUMENT", sb.toString(), "SDIDOCUMENT.USERSEQUENCE, SDIDOCUMENT.DOCUMENTID", false);
    ...
    public String getPageScripts() {
        ...
        sb.append("    _elementtype = 'SDIDocument';");
        sb.append("    sapphire.lookup.util.openWindow( 'SDIDocument',sapphire.PRODUCTNAME + ' Lookup', url, 800, 600,").append(this.isSapphireDialog()).append(" , null );");
        sb.append("}");
        sb.append("function remove").append(this.element.getId()).append("() {");
        sb.append("    _elementid = '").append(this.element.getId()).append("';");
        sb.append("    _elementtype = 'SDIDocument';");
        sb.append("    removeSDIDocument('").append(this.element.getId()).append("' );");
        sb.append("}");
        return sb.toString();
    }
