...
public class SDISecurityDepartment
extends BaseSDIDetail {
    static final String LABVANTAGE_CVS_ID = "$Revision: 79326 $";

    public SDISecurityDepartment(PropertyList element, PageContext pageContext, RequestContext requestContext, String connectionid) {
        super(element, pageContext, requestContext, connectionid);
        this._TableMetaData = OpalUtil.getColumnDataTypeMap("sdisecuritydepartment", this.getQueryProcessor());
        this.setPropertyHandlerClass("com.labvantage.opal.elements.sdidetailmaint.handler.SDISecurityDepartmentPropertyHandler");
    }
    ...
    public DataSet getDataset(RequestContext context) throws SapphireException {
        ...
        for (int i = 0; i < columnslist.size(); ++i) {
            ...
            if (this.doesColumnExists(column)) {
                ...
                c.setPrefix("SDISECURITYDEPARTMENT");
                columnList.add(c);
                continue;
            }
            String pseudovalue = (String)pseudolist.get(i);
            if (pseudovalue != null && pseudovalue.trim().length() > 0) continue;
            throw new SapphireException("Column '" + column + "' does not exists in 'SDISECURITYDEPARTMENT'.");
        }
        SafeSQL safeSQL = new SafeSQL();
        StringBuilder sb = new StringBuilder();
        sb.append(" SDISECURITYDEPARTMENT.SDCID = ").append(safeSQL.addVar(this._SdcID));
        sb.append(" AND SDISECURITYDEPARTMENT.KEYID1 = ").append(safeSQL.addVar(this.getResolvedKeyid1()));
        sb.append(" AND SDISECURITYDEPARTMENT.KEYID2 = ").append(safeSQL.addVar(this.getResolvedKeyid2()));
        sb.append(" AND SDISECURITYDEPARTMENT.KEYID3 = ").append(safeSQL.addVar(this.getResolvedKeyid3()));
        sb.append(" AND DEPARTMENT.DEPARTMENTID = SDISECURITYDEPARTMENT.SECURITYDEPARTMENT");
        String whereClause = this.element.getProperty("whereclause");
        if (whereClause != null && whereClause.trim().length() > 0) {
            sb.append(" AND ").append(whereClause);
        }
        String query = SDISecurityDepartment.getSelectSQL(columnList, "SDISECURITYDEPARTMENT, DEPARTMENT", sb.toString(), "SDISECURITYDEPARTMENT.SECURITYDEPARTMENT", false);
    ...
    protected String getPageScripts() {
        ...
        sb.append("    _elementtype = 'SDISecurityDepartment';");
        sb.append("    sapphire.lookup.util.openWindow( 'SDISecurityDepartment',sapphire.PRODUCTNAME + ' Lookup', url, 800, 600,").append(this.isSapphireDialog()).append(" , null );");
        sb.append("}");
