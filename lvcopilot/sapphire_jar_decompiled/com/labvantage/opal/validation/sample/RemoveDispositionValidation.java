...
public class RemoveDispositionValidation
    ...
    public void processRequest(HttpServletRequest request, HttpServletResponse response, ServletContext servletContext) throws ServletException {
        ...
        if (StringUtil.getLen(sampleid) > 0L) {
            ...
            if (sampleDs != null && sampleDs.size() == StringUtil.split(sampleid, ";").length) {
                this.validate("ALL", sampleDs, errorHandler, "Remove Disposition");
            } else {
                errorHandler.add(this.translate("Remove Disposition"), "", this.translate("Validation failure"), "VALIDATION", this.translate("Corrupted data: Sample(s) not available."));
            }
        }
