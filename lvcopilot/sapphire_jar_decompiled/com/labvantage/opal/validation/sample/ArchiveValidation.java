    ...
    public void processRequest(HttpServletRequest request, HttpServletResponse response, ServletContext servletContext) throws ServletException {
        ...
        if (StringUtil.getLen(sampleid) > 0L) {
            ...
            if (sampleDs != null && sampleDs.size() == StringUtil.split(sampleid, ";").length) {
                this.validate("ALL", sampleDs, errorHandler, "Archive");
            } else {
                errorHandler.add(this.getTranslationProcessor().translate("Archive"), "", this.getTranslationProcessor().translate("Validation failed"), "VALIDATION", this.getTranslationProcessor().translate("Corrupted data: Sample(s) not available"));
            }
        }
