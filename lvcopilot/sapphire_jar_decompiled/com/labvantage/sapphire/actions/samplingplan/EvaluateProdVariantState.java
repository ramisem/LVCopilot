...
import sapphire.action.EvalProdVariantState;
import sapphire.util.DataSet;
import sapphire.util.StringUtil;
import sapphire.xml.PropertyList;

public class EvaluateProdVariantState
extends BaseAction
implements EvalProdVariantState {
    ...
    public void processAction(PropertyList properties) throws SapphireException {
        ...
        if (columnMap.size() == 0) {
            ...
            valueMap.put("sdcid", tp.translate(sdcid));
            throw new SapphireException("INVALID_PARAMETERS", tp.translate("Cannot proceed, column mapping between ProdVariant and [sdcid] is not defined in the SamplingPlan policy.", valueMap));
        }
        ...
        sql.append(" FROM s_prodvariant pr, " + tableid + " sdc").append(" WHERE pr.prodvarianttype='" + prodVariantType + "' ");
        ...
        try {
            ...
            for (int i = 0; i < keyid1prop.length; ++i) {
                ...
                selectProdVars.setString(1, keyid1prop[i]);
                if (keycolid2.length() > 0) {
                    selectProdVars.setString(2, keyid2prop[i]);
                    keyIds = keyIds + "/" + keyid2prop[i];
                }
                if (keycolid3.length() > 0) {
                    selectProdVars.setString(3, keyid3prop[i]);
                ...
                block7: for (int idx = 0; idx < dsProdVars.getRowCount(); ++idx) {
                    ...
                    HashMap prodVarColValMap = SamplingPlanUtil.createProdVariantColValueMap(columnMap, dsProdVars);
                    ...
                    for (int rule = 0; rule < dsTransitionRules.getRowCount(); ++rule) {
                        ...
                        if (!SamplingPlanUtil.evaluateTransitionRule(transitionId, transitionRuleType, transitionRule, sdcid, tableid, configProcessor, this.getQueryProcessor(), tp, prodVarColValMap, lastTransitionDate, this.connectionInfo.getDbms())) continue;
