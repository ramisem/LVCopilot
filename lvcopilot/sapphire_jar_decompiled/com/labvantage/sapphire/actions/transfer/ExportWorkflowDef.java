...
public class ExportWorkflowDef
extends BaseAction
implements sapphire.action.ExportWorkflowDef,
TransferAction {
    ...
    public void processAction(PropertyList props) throws SapphireException {
        String workflowdefid = props.getProperty("workflowdefid");
        String workflowdefversionid = props.getProperty("workflowdefversionid");
        String workflowdefvariantid = props.getProperty("workflowdefvariantid");
        ...
        SDITransfer sdi = new SDITransfer("LV_WorkflowDef");
        sdi.setKeyid1(workflowdefid);
        sdi.setKeyid2(workflowdefversionid);
        sdi.setKeyid3(workflowdefvariantid);
        if (props.getProperty("workflowexists").equals("replace")) {
        ...
        sdi.setTransferOption("includetaskdefs", props.getProperty("includetaskdefs", "Y"));
        sdi.setTransferOption("taskexists", props.getProperty("taskexists", "Ignore"));
        sdi.setTransferOption("includestepdefs", props.getProperty("includestepdefs", "Y"));
        sdi.setTransferOption("nodeexists", props.getProperty("nodeexists", "ignore"));
        sdi.setTransferOption("includeconnectortypes", "Y");
        ...
        try {
            ...
            throw new SapphireException("Failed to export workflow " + LV_WorkflowDef.getText(workflowdefid, workflowdefversionid, workflowdefvariantid) + ". \nReason: " + ErrorUtil.extractMessageFromException(e, ErrorUtil.isUserAdmin(this.getConnectionId())), e);
        }
        finally {
            try {
                if (zipOut != null) {
    ...
    public PropertyList convertWizardProperties(PropertyList wizardProps, String connectionid) throws Exception {
        ...
        actionProps.setProperty("workflowdefid", wizardProps.getProperty("keyid1"));
        actionProps.setProperty("workflowdefversionid", wizardProps.getProperty("keyid2"));
        actionProps.setProperty("workflowdefvariantid", wizardProps.getProperty("keyid3"));
        actionProps.setProperty("workflowexists", wizardProps.getProperty("workflowexists"));
        ...
        actionProps.setProperty("includetaskdefs", wizardProps.getProperty("includetaskdefs"));
        actionProps.setProperty("taskexists", wizardProps.getProperty("taskexists"));
        actionProps.setProperty("includestepdefs", wizardProps.getProperty("includestepdefs"));
