...
public class AddSDIDataRelation
extends BaseAction
implements sapphire.action.AddSDIDataRelation {
    ...
    public void processAction(PropertyList properties) throws SapphireException {
        ...
        String[] paramListId = StringUtil.split(properties.getProperty("paramlistid"), PROPERTY_SEPARATOR);
        String[] paramListVersionId = StringUtil.split(properties.getProperty("paramlistversionid"), PROPERTY_SEPARATOR);
        String[] variantId = StringUtil.split(properties.getProperty("variantid"), PROPERTY_SEPARATOR);
        String[] dataSet = StringUtil.split(properties.getProperty("dataset"), PROPERTY_SEPARATOR);
        if (keyId1.length != paramListId.length) {
            throw new SapphireException("Number of sdcid's does not match paramlistid's");
        }
        if (keyId1.length != paramListVersionId.length) {
            throw new SapphireException("Number of sdcid's does not match paramlistversionid's");
        }
        if (keyId1.length != variantId.length) {
            throw new SapphireException("Number of sdcid's does not match variantid's");
        }
        if (keyId1.length != dataSet.length) {
            throw new SapphireException("Number of sdcid's does not match dataset's");
        }
        ...
        for (int item = 0; item < keyId1.length; ++item) {
            ...
            if (paramListId[item] == null || paramListId[item].length() == 0 || paramListVersionId[item] == null || paramListVersionId[item].length() == 0 || variantId[item] == null || variantId[item].length() == 0 || dataSet[item] == null || dataSet[item].length() == 0) {
                this.logger.info("Ignore item,Parameter List keys are not valid ");
                continue;
            }
            ...
            preProcessed.setString(newRow, "paramlistid", paramListId[item]);
            preProcessed.setString(newRow, "paramlistversionid", paramListVersionId[item]);
            preProcessed.setString(newRow, "variantid", variantId[item]);
            preProcessed.setNumber(newRow, "dataset", new BigDecimal(dataSet[item]));
            ...
            if (sourceKeyId1.length > item) {
                ...
                preProcessed.setString(newRow, "originalreagenttypeid", isReagent != false ? sourceKeyId1[item] : "");
            }
            if (sourceKeyId2.length > item) {
                preProcessed.setString(newRow, "sourcekeyid2", sourceKeyId2[item]);
                preProcessed.setString(newRow, "originalreagenttypeversionid", isReagent != false ? sourceKeyId2[item] : "");
            }
            if (isReagent.booleanValue() && originalReagentType.length > item && originalReagentType[item].trim().length() > 0) {
        ...
        if (preProcessed.getRowCount() > 0) {
            ...
            startId = sp.getSequence("sdidatarelation", "relationid", preProcessed.getRowCount());
        }
        for (int i = 0; i < preProcessed.getRowCount(); ++i) {
            preProcessed.setString(i, "relationid", Integer.toString(startId + i));
        }
        if (preProcessed.getRowCount() > 0) {
            String insertSQL = "INSERT INTO sdidatarelation  (sdcid, keyid1, keyid2, keyid3, paramlistid, paramlistversionid, variantid, dataset, relationid, relationtype, relationfunction,tosdcid, tokeyid1, tokeyid2, tokeyid3,refsdcid, refkeyid1, refkeyid2, refkeyid3,sourcesdcid, sourcekeyid1, sourcekeyid2, sourcekeyid3, amount, amountunits, amountunitstype, createdt,createby, mandatoryflag, requiredamount, requiredamountunits, requiredamountunitstype,relationinstance,originalreagenttypeid,originalreagenttypeversionid,amountadjusted,usedexpiredconsumableflag )  VALUES ( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?, ?,?,?,?, ?,?,?,?, ?,?,?,?,?,?,?,?,?,?,?,?,?,? )";
            ...
            try {
                for (int row = 0; row < preProcessed.getRowCount(); ++row) {
                    ...
                    ps.setString(5, preProcessed.getString(row, "paramlistid"));
                    ps.setString(6, preProcessed.getString(row, "paramlistversionid"));
                    ps.setString(7, preProcessed.getString(row, "variantid"));
                    ps.setBigDecimal(8, preProcessed.getBigDecimal(row, "dataset"));
                    ps.setString(9, preProcessed.getString(row, "relationid"));
                    ps.setString(10, preProcessed.getString(row, "relationtype"));
                    ps.setString(11, preProcessed.getString(row, "relationfunction"));
                    ps.setString(12, preProcessed.getString(row, "tosdcid"));
                    ps.setString(13, preProcessed.getString(row, "tokeyid1"));
                    ps.setString(14, preProcessed.getString(row, "tokeyid2"));
                    ps.setString(15, preProcessed.getString(row, "tokeyid3"));
                    ps.setString(16, preProcessed.getString(row, "refsdcid"));
                    ps.setString(17, preProcessed.getString(row, "refkeyid1"));
                    ps.setString(18, preProcessed.getString(row, "refkeyid2"));
                    ps.setString(19, preProcessed.getString(row, "refkeyid3"));
                    ps.setString(20, preProcessed.getString(row, "sourcesdcid"));
                    ps.setString(21, preProcessed.getString(row, "sourcekeyid1"));
                    ps.setString(22, preProcessed.getString(row, "sourcekeyid2"));
                    ps.setString(23, preProcessed.getString(row, "sourcekeyid3"));
                    BigDecimal preAmt = preProcessed.getBigDecimal(row, "amount");
                    if (preAmt != null) {
                        ps.setBigDecimal(24, preAmt);
                    } else {
                        ps.setNull(24, 2);
                    }
                    ps.setString(25, preProcessed.getString(row, "amountunits"));
                    ps.setString(26, preProcessed.getString(row, "amountunitstype"));
                    ps.setTimestamp(27, DateTimeUtil.getNowTimestamp());
                    ps.setString(28, this.connectionInfo.getSysuserId());
                    ps.setString(29, preProcessed.getString(row, "mandatoryflag"));
                    BigDecimal reqdAmt = preProcessed.getBigDecimal(row, "requiredamount");
                    if (reqdAmt != null) {
                        ps.setBigDecimal(30, reqdAmt);
                    } else {
                        ps.setNull(30, 2);
                    }
                    ps.setString(31, preProcessed.getString(row, "requiredamountunits"));
                    ps.setString(32, preProcessed.getString(row, "requiredamountunitstype"));
                    BigDecimal ins = preProcessed.getBigDecimal(row, "relationinstance");
                    if (ins != null) {
                        ps.setBigDecimal(33, ins);
                    } else {
                        ps.setNull(33, 2);
                    }
                    ps.setString(34, preProcessed.getString(row, "originalreagenttypeid"));
                    ps.setString(35, preProcessed.getString(row, "originalreagenttypeversionid"));
                    if (reqdAmt != null) {
                        ps.setBigDecimal(36, reqdAmt);
                    } else {
                        ps.setNull(36, 2);
                    }
                    ps.setString(37, preProcessed.getString(row, "usedexpiredconsumableflag"));
                    ps.executeUpdate();
                }
            }
