...
extends BaseAction {
    ...
    public static final String PROPERTY_LEVELID = "levelid";
    ...
    public void processAction(PropertyList properties) throws SapphireException {
        ...
        String levelIds = properties.getProperty(PROPERTY_LEVELID);
        ...
        if (levelIds.isEmpty()) {
            throw new SapphireException("Missing mandatory input property level ID");
        }
        ...
        String getMaxItemNoSql = "SELECT MAX(d.s_samplingplandetailno) maxdetailno FROM s_spdetail d WHERE d.s_samplingplanid = ? AND d.s_samplingplanversionid = ?";
        ...
        int maxItemNo = getMaxItemNoDs.getBigDecimal(0, "maxdetailno", BigDecimal.ZERO).intValue();
        StringBuilder detailNumbers = new StringBuilder();
        List<String> levelIdList = Arrays.asList(StringUtil.split(levelIds, ";"));
        for (String ignored : levelIdList) {
            detailNumbers.append(";").append(++maxItemNo);
        }
        ...
        addSamplingPlanDetailProps.setProperty("linkid", "detail");
        addSamplingPlanDetailProps.setProperty(PROPERTY_LEVELID, levelIds);
        addSamplingPlanDetailProps.setProperty("s_samplingplandetailno", detailNumbers.substring(1));
