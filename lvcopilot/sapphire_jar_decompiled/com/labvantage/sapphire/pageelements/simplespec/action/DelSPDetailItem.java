...
extends BaseAction {
    ...
    public void processAction(PropertyList properties) throws SapphireException {
        ...
        Object[] getItemNumbersParams = new Object[]{samplingPlanId, samplingPlanVersionId};
        DataSet getItemNumbersDs = this.getQueryProcessor().getPreparedSqlDataSet(getItemNumbersSql, getItemNumbersParams);
        ...
        for (int i = 0; i < workItemIdList.size(); ++i) {
            ...
            for (int j = 0; j < getItemNumbersDs.getRowCount(); ++j) {
                ...
                itemNumbers.append(";").append(getItemNumbersDs.getBigDecimal(j, "s_samplingplanitemno").toPlainString());
            }
        }
        PropertyList deleteDetailItemProps = new PropertyList();
        deleteDetailItemProps.setProperty(PROPERTY_SDCID, "LV_SamplingPlan");
        deleteDetailItemProps.setProperty(PROPERTY_KEYID1, samplingPlanId);
        deleteDetailItemProps.setProperty(PROPERTY_KEYID2, samplingPlanVersionId);
        deleteDetailItemProps.setProperty("linkid", "item");
        deleteDetailItemProps.setProperty("detaillinkid", "detail");
        deleteDetailItemProps.setProperty("s_samplingplanitemno", itemNumbers.substring(1));
        deleteDetailItemProps.setProperty("s_samplingplandetailno", samplingPlanDetailNumbers);
        this.getActionProcessor().processAction("DeleteSDIDetail", "1", deleteDetailItemProps);
    }
}
