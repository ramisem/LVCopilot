...
public class PropertyBuilderPropertiesAjaxRender
        ...
        if (props.length() > 0) {
            try {
                ...
                try {
                    ...
                    String propsHtml = PropertyBuilder.getPropertiesHtml(prop, ajaxResponse.getRequestParameter("viewonly", "n").equalsIgnoreCase("y"), ajaxResponse.getRequestParameter("definitionxml", ""), this.getConnectionId(), RequestContext.getInstance(request).getPropertyList("userconfig"), this.getTranslationProcessor(), request.getSession(), ProcessingUtil.createBindingsMap(null, this.getQueryProcessor(), this.getSDCProcessor(), null, null));
