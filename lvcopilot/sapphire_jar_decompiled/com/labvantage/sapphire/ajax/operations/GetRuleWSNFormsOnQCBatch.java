...
public class GetRuleWSNFormsOnQCBatch
extends BaseAjaxRequest {
    ...
    public void processRequest(HttpServletRequest request, HttpServletResponse response, ServletContext servletContext) throws ServletException {
        ...
        String qcbatchid = ajaxResponse.getRequestParameter("qcbatchid").replaceAll("%3B", ";");
        String rsetid = "";
        boolean hasWS = false;
        try {
            rsetid = this.getDAMProcessor().createRSet("QCBatch", qcbatchid, null, null);
        }
        catch (SapphireException e) {
            ajaxResponse.setError(e.getMessage());
        }
        SafeSQL safeSQL = new SafeSQL();
        String sql4WS = "SELECT count(b.s_qcbatchid) FROM s_qcbatch b, rsetitems r WHERE r.rsetid in (" + safeSQL.addIn(rsetid.replaceAll(";", "', '")) + ") AND r.sdcid = 'QCBatch' AND  r.keyid1 =b.s_qcbatchid AND b.documentid is not null";
        try {
        ...
        String keyid = "";
        for (int i = 0; i < StringUtil.split(qcbatchid, ";").length; ++i) {
            keyid = keyid + ";(null)";
        }
        actionProps.setProperty("keyid2", keyid);
        actionProps.setProperty("keyid3", keyid);
        ...
        ajaxResponse.addCallbackArgument("hasWS", hasWS);
