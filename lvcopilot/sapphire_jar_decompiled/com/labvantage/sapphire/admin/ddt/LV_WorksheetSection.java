...
public class LV_WorksheetSection
...
implements ELNConstants {
    ...
    public void preAdd(SDIData sdiData, PropertyList actionProps) throws SapphireException {
        if ("N".equals(actionProps.getProperty("worksheet_action", "N")) && "N".equals(actionProps.getProperty("importsnapshot", "N"))) {
            throw new SapphireException("New worksheet sections must be created with the AddWorksheetSection action or Imported");
        }
        DataSet primary = sdiData.getDataset("primary");
        if (primary.isValidColumn("worksheetsectiondesc")) {
            for (int i = 0; i < primary.getRowCount(); ++i) {
                String worksheetsectiondesc = primary.getString(i, "worksheetsectiondesc", "");
                if (worksheetsectiondesc.length() <= 80) continue;
                worksheetsectiondesc = worksheetsectiondesc.substring(0, 77) + "...";
                primary.setString(i, "worksheetsectiondesc", worksheetsectiondesc);
            }
        }
    }
    ...
    public void preCMTImport(SDIData sdiData, PropertyList actionProps, boolean isAddSDI) throws SapphireException {
        DataSet primary = sdiData.getDataset("primary");
        LV_Worksheet.resetSequence(primary, this.getSequenceProcessor(), "worksheetsectionid");
    }
}
