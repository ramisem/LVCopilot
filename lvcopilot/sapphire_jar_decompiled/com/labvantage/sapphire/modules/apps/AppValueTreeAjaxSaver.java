...
import com.labvantage.sapphire.modules.apps.AppValueTree;
...
public class AppValueTreeAjaxSaver
extends BaseAjaxRequest {
    ...
    public void processRequest(HttpServletRequest request, HttpServletResponse response, ServletContext servletContext) throws ServletException {
        ...
        if (props_pvt.length() > 0 && props_vt.length() > 0) {
            ...
            try {
                PropertyList appprops_pvt = new PropertyList(new JSONObject(props_pvt));
                PropertyList appprops_cvt = new PropertyList(new JSONObject(props_cvt));
                PropertyList appprops_vt = new PropertyList(new JSONObject(props_vt));
                String appid = ajaxResponse.getRequestParameter("appid");
                boolean isCompMode = false;
                if (appid.length() == 0) {
                    isCompMode = configCompCode.length() > 0;
                } else if (configCompCode.length() > 0) {
                    DataSet ds = this.getQueryProcessor().getPreparedSqlDataSet("SELECT compcode FROM app WHERE appid=?", (Object[])new String[]{appid});
                    if (ds.size() == 0) {
                        isCompMode = configCompCode.length() > 0;
                    } else {
                ...
                PropertyList out = AppValueTree.getProperties(appprops_pvt, appprops_cvt, appprops_vt, appid, devMode, isCompMode, compOverride, false, pageContext, this.logger);
