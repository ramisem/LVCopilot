...
extends BaseSDCEventType {
    ...
    public Class[] getEventObjectImplementations() {
        return new Class[]{PostAddEventObject.class, PostAddWorkItemEventObject.class};
    }

    @Override
    public boolean requiresSupplementalData(DataSet conditionValues) {
        for (int i = 0; i < conditionValues.size(); ++i) {
            if (!conditionValues.getValue(i, "conditionitem").contains("_") || conditionValues.getValue(i, "conditionitem").startsWith("sdiworkitem")) continue;
            return true;
        }
        return false;
    }
    ...
    public Condition getFilterConditionTemplate(DataSet setupConditionValues) {
        if (setupConditionValues != null && setupConditionValues.size() > 0) {
            ...
            conditionItems.add(this.getEventSourceConditionItem(new String[]{"PostAddWorkItem"}));
    ...
    public boolean hasEventFired(BaseEventObject eventObject, DataSet eventplanconditions) {
        return this.hasSDIXXXCreatedEventFired(eventObject, "sdiworkitem", "sdiworkitem", eventplanconditions);
    }
