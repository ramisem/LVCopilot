...
extends BaseSDCEventType {
    ...
    private static int SPECCHECKCONDITION_INDEX = 2;
    ...
    private static int SPECCHECKTYPE_INDEX = 3;
    ...
    public String[] getSupplementalRequestItems(DataSet conditionValues) {
        return new String[]{"primary", "sdispec", "dataspec"};
    }
    ...
    public Condition getSetupCondition(int index, DataSet setupConditionValues) {
        ...
        if (index == SPECCHECKTYPE_INDEX) {
            ...
            editorProps = new PropertyList();
            editorProps.setProperty("mode", "dropdownlist");
            editorProps.setProperty("displayvalue", "A=Any");
    ...
    public Condition[] getSetupConditions(DataSet eventTypeSetupConditions) {
        ...
        if (eventTypeSetupConditions != null && eventTypeSetupConditions.size() >= 1 && eventTypeSetupConditions.getValue(0, "conditionitem").equals("sdcid") && eventTypeSetupConditions.getValue(0, "operator1").equals("equals") && eventTypeSetupConditions.getValue(0, "value1").length() > 0) {
            if (eventTypeSetupConditions.size() >= 2 && eventTypeSetupConditions.getValue(1, "conditionitem").equals(SPECCHECKSOURCE_ID) && eventTypeSetupConditions.getValue(1, "operator1").equals("equals") && eventTypeSetupConditions.getValue(1, "value1").length() > 0) {
                if (eventTypeSetupConditions.size() >= 3 && eventTypeSetupConditions.getValue(2, "conditionitem").equals(SPECCHECKCONDITION_ID) && eventTypeSetupConditions.getValue(2, "operator1").equals("equals") && eventTypeSetupConditions.getValue(2, "value1").length() > 0) {
                    if (eventTypeSetupConditions.size() >= 4 && eventTypeSetupConditions.getValue(3, "conditionitem").equals(SPECCHECKTYPE_ID) && eventTypeSetupConditions.getValue(3, "operator1").equals("equals") && eventTypeSetupConditions.getValue(3, "value1").length() > 0) {
                        return null;
                    }
            ...
            conditions = new Condition[]{new Condition(new ConditionItem(SPECCHECKSOURCE_ID, SPECCHECKSOURCE_DISPLAY, "equals", true)), new Condition(new ConditionItem(SPECCHECKCONDITION_ID, SPECCHECKCONDITION_DISPLAY, "equals", true)), new Condition(new ConditionItem(SPECCHECKTYPE_ID, SPECCHECKTYPE_DISPLAY, "equals", true))};
            return conditions;
        }
        conditions = new Condition[]{new Condition(this.getSDCConditionItem()), new Condition(new ConditionItem(SPECCHECKSOURCE_ID, SPECCHECKSOURCE_DISPLAY, "equals", true)), new Condition(new ConditionItem(SPECCHECKCONDITION_ID, SPECCHECKCONDITION_DISPLAY, "equals", true)), new Condition(new ConditionItem(SPECCHECKTYPE_ID, SPECCHECKTYPE_DISPLAY, "equals", true))};
        return conditions;
    }
    ...
    public Condition getFilterConditionTemplate(DataSet setupConditionValues) {
        if (setupConditionValues != null && setupConditionValues.size() > 0) {
            ...
            conditionItems.addAll(this.getSDCColumnConditionItems("DataItem"));
            conditionItems.add(new ConditionItem("sdispec_oosgeneratingflag", "OOS Generating Flag", "S", ""));
    ...
    public boolean hasEventFired(BaseEventObject eventObject, DataSet eventplanconditions) {
        ...
        if (eventObject instanceof PostDataReleaseEventObject && (dataReleaseEventObject = (PostDataReleaseEventObject)eventObject).isRelease() && dataReleaseEventObject.getProperties().getProperty("dataentryautorelease", "N").equals("N")) {
            ...
            DataSet sdidataitemspec = dataReleaseEventObject.getDataSet("dataspec");
            DataSet sdispec = dataReleaseEventObject.getDataSet("sdispec");
            DataSet sdidataitem = dataReleaseEventObject.getDataSet("dataitem");
            if (sdidataitemspec != null && eventObject.hasEventItems() && eventplanconditions.size() >= 3) {
                String sdcid = dataReleaseEventObject.getSdcid();
                String primaryTableid = this.sdcProps.getProperty("tableid");
                if (eventplanconditions.getValue(0, "value1").equals(sdcid) && eventplanconditions.getValue(1, "value1").equals(eventObject.getEventid())) {
                    boolean anySpec = eventplanconditions.getValue(3, "value1", SPECCHECKTYPE_ANYSPEC).equals(SPECCHECKTYPE_ANYSPEC);
                    if (!anySpec) {
                        // empty if block
                    }
                    HashMap<String, Object> valueMap = new HashMap<String, Object>();
                    valueMap.put("condition", eventplanconditions.getValue(SPECCHECKCONDITION_INDEX, "value1"));
                    if (sdidataitemspec.findRow(valueMap) > -1) {
                        DataSet matchingSDIDataItemSpec = sdidataitemspec.getFilteredDataSet(valueMap);
                        ArrayList<BaseEventItem> eventItems = eventObject.getEventItems();
                        int eventItemsFired = eventItems.size();
                        for (int i = 0; i < eventItems.size(); ++i) {
                            boolean isTemplate;
                            DataItemEventItem eventItem = (DataItemEventItem)eventItems.get(i);
                            ...
                            boolean currentVersionsResolved = false;
                            for (int j = 2; eventItem.hasFired() && j < eventplanconditions.size(); ++j) {
                                if (j == SPECCHECKCONDITION_INDEX) {
                                    ...
                                    valueMap.put("paramlistid", eventItem.getParamlistid());
                                    valueMap.put("paramlistversionid", eventItem.getParamlistversionid());
                                    valueMap.put("variantid", eventItem.getVariantid());
                                    valueMap.put("dataset", new BigDecimal(eventItem.getDataset()));
                                    valueMap.put("paramid", eventItem.getParamid());
                                    valueMap.put("paramtype", eventItem.getParamtype());
                                    valueMap.put("replicateid", new BigDecimal(eventItem.getReplicateid()));
                                    if (matchingSDIDataItemSpec.findRow(valueMap) != -1) continue;
                                    eventItem.setHasFired(false);
                                    --eventItemsFired;
                                    continue;
                                }
                                if (j == SPECCHECKTYPE_INDEX) continue;
                                if (!currentVersionsResolved) {
                                    String paramlistid = "";
                                    String paramlistversionid = "";
                                    String variantid = "";
                                    int index = -1;
                                    for (int k = j; k < eventplanconditions.size(); ++k) {
                                        String conditionItem = eventplanconditions.getValue(k, "conditionitem");
                                        if (conditionItem.equalsIgnoreCase("sdidataitem_paramlistid")) {
                                            paramlistid = eventplanconditions.getValue(k, "value1");
                                            continue;
                                        }
                                        if (conditionItem.equalsIgnoreCase("sdidataitem_paramlistversionid")) {
                                            paramlistversionid = eventplanconditions.getValue(k, "value1");
                                            index = k;
                                            continue;
                                        }
                                        if (!conditionItem.equalsIgnoreCase("sdidataitem_variantid")) continue;
                                        variantid = eventplanconditions.getValue(k, "value1");
                                    }
                                    if (paramlistid.length() > 0 && variantid.length() > 0 && (paramlistversionid.length() == 0 || paramlistversionid.equalsIgnoreCase("C"))) {
                                        paramlistversionid = this.resolveParamListVersion(paramlistid, paramlistversionid, variantid);
                                        eventplanconditions.setValue(index, "value1", paramlistversionid);
                                        currentVersionsResolved = true;
                                    }
                                }
                                String conditionItem = eventplanconditions.getValue(j, "conditionitem");
                                if (sdidataitemspec != null && conditionItem.startsWith("sdidataitemspec_") && !this.conditionMatch(sdidataitemspec, eventItem.getDataIndex(), "sdidataitemspec", eventplanconditions, j)) {
                                    eventItem.setHasFired(false);
                                    --eventItemsFired;
                                    continue;
                                }
                                ...
                                if (sdidataitem != null && conditionItem.startsWith("sdidataitem_") && !this.conditionMatch(sdidataitem, eventItem.getDataIndex(), "sdidataitem", eventplanconditions, j)) {
                                    eventItem.setHasFired(false);
                                    --eventItemsFired;
                                    continue;
                                }
    ...
    public HashMap getProcessingInputs(BaseEventObject eventObject, String[] inputTokens) {
        if (eventObject instanceof BaseSDCEventObject) {
            ...
            tokenValues.putAll(this.getTableTokenValues(eventObject, "dataitem", "sdidataitem", inputTokens));
            return tokenValues;
        }
        return null;
    }
