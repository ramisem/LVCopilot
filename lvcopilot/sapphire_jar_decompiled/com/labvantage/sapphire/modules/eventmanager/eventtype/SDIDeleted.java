...
public class SDIDeleted
extends BaseSDCEventType {
    @Override
    public Class[] getEventObjectImplementations() {
        return new Class[]{PostDeleteEventObject.class};
    }
    ...
    public Condition getFilterConditionTemplate(DataSet setupConditionValues) {
        if (setupConditionValues != null && setupConditionValues.size() > 0) {
            ...
            conditionItems.add(this.getEventSourceConditionItem(new String[]{"PostDelete"}));
    ...
    public boolean hasEventFired(BaseEventObject eventObject, DataSet eventplanconditions) {
        if (eventObject instanceof PostDeleteEventObject) {
            DataSet primary = ((PostDeleteEventObject)eventObject).getDataSet("primary");
            if (eventObject.hasEventItems() && eventplanconditions.size() >= 1) {
                ...
                if (eventplanconditions.getValue(0, "value1").equals(sdcid)) {
                    if (primary != null && eventplanconditions.size() > 1) {
    ...
    public HashMap getProcessingInputs(BaseEventObject eventObject, String[] inputTokens) {
        if (eventObject instanceof PostDeleteEventObject) {
            PostDeleteEventObject postDeleteEventObject = (PostDeleteEventObject)eventObject;
            return this.getPrimaryTokenValues(postDeleteEventObject.getSdcid(), eventObject, inputTokens);
        }
        return null;
    }
}
