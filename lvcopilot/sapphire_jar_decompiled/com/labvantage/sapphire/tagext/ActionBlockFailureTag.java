...
public class ActionBlockFailureTag
extends BaseBodyTagSupport {
    public int doStartTag() throws JspTagException {
        int rc = 0;
        ActionBlockTag _actionblock = null;
        _actionblock = (ActionBlockTag)ActionBlockFailureTag.findAncestorWithClass((Tag)this, ActionBlockTag.class);
        if (_actionblock == null) {
            this.write("TAG ERROR: actionblocksuccess tag must be nested in a actionblock tag.<br>");
        } else {
            _actionblock.processBlock();
            if (!_actionblock.isSuccessful()) {
                rc = 2;
            }
        }
        return rc;
