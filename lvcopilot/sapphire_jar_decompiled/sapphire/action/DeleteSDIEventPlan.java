/*
 * Decompiled with CFR 0.152.
 */
package sapphire.action;

public interface DeleteSDIEventPlan {
    public static final String ID = "DeleteSDIEventPlan";
    public static final String VERSIONID = "1";
    public static final String PROPERTY_SDCID = "sdcid";
    public static final String PROPERTY_KEYID1 = "keyid1";
    public static final String PROPERTY_KEYID2 = "keyid2";
    public static final String PROPERTY_KEYID3 = "keyid3";
    public static final String PROPERTY_EVENTPLANID = "eventplanid";
    public static final String PROPERTY_EVENTPLANVERSIONID = "eventplanversionid";
    public static final String PROPERTY_EVENTPLANSDCID = "eventplansdcid";
    public static final String PROPERTY_RSETID = "rsetid";
    public static final String PROPERTY_APPLYLOCK = "applylock";
}

